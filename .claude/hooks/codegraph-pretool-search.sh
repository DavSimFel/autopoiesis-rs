...
CODE_GRAPH_BIN=$(command -v code-graph 2>/dev/null || true)
if [ -z "$CODE_GRAPH_BIN" ] && [ -x "/home/user/.cargo/bin/code-graph" ]; then
  CODE_GRAPH_BIN="/home/user/.cargo/bin/code-graph"
fi
if [ -z "$CODE_GRAPH_BIN" ] && [ -x "./scripts/code-graph" ]; then
  CODE_GRAPH_BIN="./scripts/code-graph"
fi
if [ -z "$CODE_GRAPH_BIN" ]; then
  exit 0
fi
...
GRAPH_CONTEXT=$("$CODE_GRAPH_BIN" find -- "$PATTERN" . 2>/dev/null | head -30)

# No results or empty array — let native search proceed without context
if [ -z "$GRAPH_CONTEXT" ] || [ "$GRAPH_CONTEXT" = "[]" ]; then
  exit 0
fi

# Return additionalContext with graph results
# Must include permissionDecision for valid PreToolUse output
jq -n \
  --arg pattern "$PATTERN" \
  ...
  '{
    "hookSpecificOutput": {
      ...
      "permissionDecisionReason": "enriched with code-graph context",

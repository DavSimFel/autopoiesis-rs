...
use crate::gate::{
    BudgetSnapshot, GuardContext, GuardEvent, SecretRedactor, Severity, Verdict as GuardResult,
};
use crate::llm::MessageContent;
use crate::llm::{ChatMessage, ChatRole, FunctionTool, ToolCall};
...
#[test]
fn later_deny_overrides_earlier_approval() {
    let hits = Arc::new(Mutex::new(Vec::<&'static str>::new()));
    let turn = Turn::new()
        .guard(RecordingGuard::new(
            "requester",
    ...
    assert!(matches!(result, GuardResult::Deny { gate_id, .. } if gate_id == "blocker"));
    assert_eq!(observed, vec!["requester", "blocker"]);
}
...
#[test]
fn highest_severity_approval_wins() {
    let hits = Arc::new(Mutex::new(Vec::<&'static str>::new()));
    let turn = Turn::new()
        .guard(RecordingGuard::new(
            "low",
    ...
    assert!(matches!(result, GuardResult::Approve { gate_id, .. } if gate_id == "high"));
    assert_eq!(observed, vec!["low", "high"]);
}
...
#[test]
fn build_turn_for_config_with_subscriptions_preserves_t2_subscriptions() {
    let root = std::env::temp_dir().join(format!(
        "autopoiesis_turn_t2_subscription_{}",
        SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_nanos()
    ...
    let config = test_config_for_tier(Some("t2"), None);
    let turn = build_turn_for_config_with_subscriptions(
        &config,
        &[SubscriptionRecord {
            id: 1,
...
#[test]
fn build_turn_for_config_with_subscriptions_preserves_t3_subscriptions() {
    let root = std::env::temp_dir().join(format!(
        "autopoiesis_turn_t3_subscription_{}",
        SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_nanos()
